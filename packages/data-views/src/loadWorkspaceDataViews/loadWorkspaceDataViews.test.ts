...
import { loadWorkspaceDataViews } from './loadWorkspaceDataViews';
...
// A view file in the second workspace
const otherView = { ...dataView_gallery_1, id: 'data-view_gallery_other' };
const otherViewPath = Fs.concatPath(
  workspace_2.path,
  Paths.hiddenDirName,
  ViewsDirName,
  `${otherView.id}.json`,
...
describe('loadWorkspaceDataViews', () => {
  beforeEach(() => {
    setup({ loadViews: false });

    // Add a view file to a workspace other than the active one
    MockFs.addFiles([
      { path: otherViewPath, textContent: JSON.stringify(otherView) },
    ]);
  });
  ...
  it('loads views into the store', async () => {
    await loadWorkspaceDataViews(workspace_1);
  ...
  it("loads views into the workspace's store record", async () => {
    await loadWorkspaceDataViews(workspace_2);

    // Should load into the second workspace's record, not the
    // active workspace's.
    expect(DataViewsStore.in(workspace_2.id).get(otherView.id)).toEqual({
      ...otherView,
      references: [],
    });
    expect(DataViewsStore).not.toHaveItem(otherView.id);
  });
