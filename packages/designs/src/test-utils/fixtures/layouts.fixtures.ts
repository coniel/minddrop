import { Layout, LayoutId, LayoutType } from '../../types';
import {
  element_container_1,
  element_text_2,
...
function generateLayoutFixture(type: LayoutType, version: number): Layout {
  const id: LayoutId = `layout_${type}-${version}`;
...
export const layout_space_1 = generateLayoutFixture('space', 1);
