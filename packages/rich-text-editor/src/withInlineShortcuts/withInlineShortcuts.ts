...
import { Editor as SlateEditor, Path, Text, Transforms } from 'slate';
import {
  Editor,
  InlineShortcut,
...
export function withInlineShortcuts(
  ...
  const triggerChars = shortcuts.reduce(
  ...
  // of every shortcut.
  const triggerActions = shortcuts.reduce(
    (value, shortcut) => [
      ...value,
      ...shortcut.triggers.map((trigger) => [trigger, shortcut.action]),
    ],
    [],
  );
