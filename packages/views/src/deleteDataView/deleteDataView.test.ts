...
describe('deleteDataView', () => {
  ...
  it('removes the view from the store', async () => {
    await deleteDataView(view_gallery_1.id);

    expect(DataViewsStore.get(view_gallery_1.id)).toBeNull();
  });
  ...
  it('does not delete a file for virtual views', async () => {
    ...
    await deleteDataView(view_virtual_1.id);
  ...
  it('dispatches a view deleted event', async () =>
    new Promise<void>((done) => {
      ...
      deleteDataView(view_gallery_1.id);
    }));
});
