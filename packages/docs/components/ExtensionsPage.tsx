...
export function ExtensionsPage({ children }: { children: React.ReactNode }) {
  ...
  const currentPageIndex = allExtensionsRoutes.findIndex(
    (page) => page.slug === currentPageSlug,
  );
  const previous = allExtensionsRoutes[currentPageIndex - 1];
  const next = allExtensionsRoutes[currentPageIndex + 1];
    ...
    >
      ...
      >
        <ScrollArea>
          ...
          >
            {extensionsRoutes.map((section: RouteProps) => (
