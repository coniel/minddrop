...
Opens the topic view for a given topic.

```ts
App.openTopicView(core: Core, topicId: string): void
```
  ...
  data={[
    {
      ...
      type: 'string',
      required: true,
      description: 'The ID of the topic for which to open the view.',
    },
  ]}
/>
...
Returns a list of views leading up to and including the provided topic based on the topic's hierarchy of parents.

```ts
App.getTopicBreadcrumbs(topidId: string): ResourceView[]
```
  ...
  data={[
    {
      ...
      required: true,
      description:
        'The ID of the topic for which to retreive the breadcrumbs.',
    },
  ]}
/>
