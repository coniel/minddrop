...
Adds subtopics into a parent topic. Dispatches an `topics:add-subtopics` event, as well as `topics:update` event.

```ts
Topics.addSubtopics(core: Core, topicId: string, subtopicIds: string[]): void
```
...
Removes subtopics from a topic. Dispatches a `topics:remove-subtopics` event as well as `topics:update` event.

```tsx
Topics.removeSubtopics(core: Core, parentId: string, subtopicIds: string[]): void
```
...
Adds drops to a topic, dispaches an `topics:add-drops` event and an `topics:update` event.

```ts
Topics.addDrops(core: Core, topicId: string, dropIds: string[]): void
```
...
Removes drops from a topic and dispatches a `topics:remove-drops` event as well as an `topics:update` event.
...
Adds tags to a topic, dispaches an `topics:add-tags` event and an `topics:update` event.

```ts
Topics.addTags(core: Core, topicId: string, tagIds: string[]): void
```
...
Removes tags from a topic and dispatches a `topics:remove-tags` event as well as an `topics:update` event.

```ts
Topics.addTag(core: Core, topicId: string, tagIds: string[]): void
```
...
Archives a topic and dispatches a `topics:archive` event and an `topics:update` event.
...
Unarchives a topic and dispatches a `topics:unarchive` event and an `topics:update` event.
...
Deletes a topic and dispatches a `topics:delete` event and an `topics:update` event.
...
Restores a deleted topic and dispatches a `topics:restore` event and an `topics:update` event.
