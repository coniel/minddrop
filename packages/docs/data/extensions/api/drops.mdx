...
Retrieves drops by ID. If provided a signle ID string, returns the drop or `null` if not found. If provided an array of IDs, returns an array of the corresponding drops.
...
Unarchives a drop and dispatches a `drops:unarchive` event and an `drops:update` event.
...
Restores a deleted drop and dispatches a `drops:restore` event and an `drops:update` event.
