...
function runOnQueryChange(
  ...
  onQueryChange: InlineMenuConfig['onQueryChange'],
) {
  if (!editor.selection) {
    return;
  }
...
export function withInlineMenus(
  ...
  // Create a `{ [keyCode]: InlineMenuConfig }` map of
  // trigger character key codes and their corresponding
  // inline menu config.
  const triggerActions: Record<string, InlineMenuConfig> =
