...
export function runSchemaCreateHooks(
  ...
  // Add the document as a parent on resources referenced
  ...
  Object.keys(schema).forEach((key) => {
    ...
    if (validator.type === 'resource-id' && validator.addAsParent) {
      ...
      if (referencedDocumentId) {
        ...
        resource.addParents(core, document[key], [parentRef]);
      }
