...
itself, and `initializeWorkspaces` reads it back directly. Three reasons,
any one of which is enough:

- The Bun process reads the file before any webview exists.
  `hasWorkspace` (`apps/desktop-electrobun/src/bun/windows/hasWorkspace.ts`)
  parses it with `Bun.file` to decide whether to open the onboarding
  window or the main one. It has no event bus and no file system adapter,
  so it cannot go through a store; making the file a store's output would
  only mean the Bun side parsing a format owned by `packages/stores`.
- The stores directory is where losable state lives — theme, sidebar
  widths, panel sizes. The workspace list is not losable: without it the
  user re-adds every workspace by hand, even though the directories are
  all still on disk.
- The onboarding window, which is where workspaces are first created,
  registers no store persist listeners at all
  (`registerAppDataStoreListeners` lives in `features/desktop-app`, and
  features cannot import features). A store write there would go nowhere.

The active workspace path rides along in the same file for the same
reasons, which is why `ActiveWorkspaceStore` is not persisted and
`setActiveWorkspace` writes the config itself. Persisting only that store
while leaving the paths list alone would be a genuine simplification —
it would make `setActiveWorkspace` synchronous and drop the write
ordering that `createWorkspace` and `addWorkspace` have to respect — but
it needs the third point solved first.
