...
// Fallback preview height for layouts without a fixed frame height
...
export interface DesignDashboardProps {
  /**
   * Callback fired with the design to open when a design card is
   * clicked or a new design is created.
   */
...
export const DesignDashboard: React.FC<DesignDashboardProps> = ({
  ...
  const handleCreateDesign = useCallback(async () => {
    const design = await Designs.create({ type: 'database' });

    onOpenDesign(design);
  }, [onOpenDesign]);

  return (
    <div className="designs-dashboard">
      <Group gap={2} className="designs-dashboard-header">
        {onClickBack && (
          <IconButton
            icon="arrow-left"
      ...
      <ScrollArea>
        <div className="designs-dashboard-content">
          {designs.length === 0 && (
            <Text block color="muted" text="design-studio.dashboard.empty" />
          )}
          <div className="designs-dashboard-grid">
            {designs.map((design) => (
              <DesignCard
                key={design.id}
                design={design}
...
const DesignCard: React.FC<DesignCardProps> = ({ design, onClick }) => {
  ...
  return (
    <button
      type="button"
      className="designs-dashboard-card"
      onClick={handleClick}
    >
      <DesignCardPreview design={design} />
      <Stack gap={1} className="designs-dashboard-card-details">
...
const DesignCardPreview: React.FC<DesignCardPreviewProps> = ({ design }) => {
  ...
  return (
    <div ref={containerRef} className="designs-dashboard-card-preview">
      {bounds ? (
              ...
              >
                {design.layouts.map((layout) => (
                  <div
                    ...
                    className="designs-dashboard-card-preview-layout"
                    style={{
...
/**
 * Returns a layout's measured height, falling back to its frame
 * height before it has been measured. Content-sized layouts with
 * no frame height use a fixed estimate.
 */
function getLayoutHeight(
  ...
  return (
    layoutHeights[layout.id] ?? layout.frame.height ?? PREVIEW_FALLBACK_HEIGHT
  );
}
