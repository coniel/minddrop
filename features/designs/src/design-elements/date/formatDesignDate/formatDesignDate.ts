...
export function formatDesignDate(
  ...
  if (resolved.mode === 'relative') {
    ...
    return getRelativeString(date, reference, locale);
  }
 ...
 */
function getRelativeString(date: Date, now: Date, locale?: string): string {
  const diffMs = now.getTime() - date.getTime();
  const absDiffMs = Math.abs(diffMs);
  const isPast = diffMs > 0;

  // Break the difference down into calendar-ish units
  ...
  // Under a minute reads as the locale's "now"
  if (seconds < 60) {
  ...
  // Pick the largest unit the difference fills
  if (minutes < 60) {
    value = minutes;
    unit = 'minute';
  } else if (hours < 24) {
