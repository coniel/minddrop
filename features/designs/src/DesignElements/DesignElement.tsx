...
export const DesignElement: React.FC<DesignElementProps> = ({ element }) => {
  ...
  // Wrap in a data-element-id div for DOM querying (e.g. selection
  // overlays). Uses display:contents so it doesn't affect layout. The
  // transient state scope keys stateful content by its ancestor element
  // chain so identical components in sibling elements cannot collide.
  return (
