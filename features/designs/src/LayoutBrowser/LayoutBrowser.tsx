...
import { LayoutBrowserList } from './LayoutBrowserList';
...
export const LayoutBrowser: React.FC<LayoutBrowserProps> = ({
  databaseId,
  backEvent,
  backEventData,
}) => {
  ...
  const selectedDesign = Layouts.use(selectedDesignId || '');
  ...
  useEffect(() => {
    ...
    const listenerId = 'layout-property-mapping';
  ...
  useEffect(() => {
    ...
    const layouts = Layouts.getAll();
    const firstLayout = layouts.find(
      (layout) => !defaultLayoutIds.includes(layout.id),
    );

    if (firstLayout) {
      selectDesign(firstLayout.id);
    }
  ...
  return (
    <div ref={browserRef} className="layout-browser">
      {/* List panel */}
      <Panel className="layout-browser-list-panel">
        {view === 'browse' ? (
          <LayoutBrowserList
      ...
      <div className="layout-browser-preview-panel">
        {selectedDesign && (
          <>
            {/* Action bar at the top of the workspace */}
            {view === 'browse' && (
              <div className="layout-browser-action-bar">
                <Button
                  label="designs.layout-property-mapping.browser.useLayout"
                  variant="solid"
                  color="primary"
                  size="lg"
            ...
            {view === 'map-properties' && (
              <div className="layout-browser-action-bar">
                ...
                <Group gap={5}>
                  <SwitchField
                    label="designs.layout-property-mapping.browser.zoom"
                    size="sm"
                    checked={zoomEnabled}
                    onCheckedChange={setZoomEnabled}
                  />
                  <SwitchField
                    label="designs.layout-property-mapping.browser.highlightMapped"
                    size="sm"
                    checked={highlightMapped}
                    onCheckedChange={setHighlightMapped}
                  />
                  <SwitchField
                    label="designs.layout-property-mapping.browser.highlightUnmapped"
                    size="sm"
                ...
                >
                  {t('designs.layout-property-mapping.browser.mappingCounter', {
                    mapped: mappedCount,
            ...
            <div
              ref={canvasAreaRef}
              className="layout-browser-canvas-area"
              style={{
