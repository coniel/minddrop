...
import {
  ...
  ItemTypePropertyEditorProps,
} from './ItemTypePropertyEditor';

const property = ItemTypesFixtures.urlItemTypeConfig.properties[0];
const property2 = ItemTypesFixtures.urlItemTypeConfig.properties[1];

describe('<ItemTypePropertyEditor />', () => {
  beforeEach(setup);
  ...
  describe('existing property', () => {
    async function updatePropertyIcon() {
      ...
      render(
        <ItemTypePropertyEditor
          itemType={ItemTypesFixtures.urlItemTypeConfig.nameSingular}
          property={property}
        />,
      );
    ...
    it('updates the property on save', () =>
      new Promise<void>((done) => {
        // Listen for item type updates and verify the property was updated
        Events.addListener<ItemTypeConfig>(
          'item-types:item-type:update',
          'test',
          ({ data }) => {
            expect(
    ...
    describe('deleting', () => {
      ...
      it('confirms before deleting the property', () =>
        new Promise<void>((done) => {
          ...
          // Listen for item type updates and verify the property was deleted
          Events.addListener<ItemTypeConfig>(
            'item-types:item-type:update',
            'test',
            ({ data }) => {
  ...
  describe('draft property', () => {
    function renderDraftEditor(
      props: Partial<ItemTypePropertyEditorProps> = {},
    ) {
      render(
        <ItemTypePropertyEditor
          isDraft
    ...
    it('adds the property on save', () =>
      new Promise<void>((done) => {
        // Listen for item type updates and verify the property was added
        Events.addListener<ItemTypeConfig>(
          'item-types:item-type:update',
          'test',
          ({ data }) => {
