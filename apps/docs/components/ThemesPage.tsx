...
export function ThemesPage({ children }: { children: React.ReactNode }) {
  ...
  const currentPageIndex = allthemesRoutes.findIndex(
    (page) => page.slug === currentPageSlug,
  );
  const previous = allthemesRoutes[currentPageIndex - 1];
  const next = allthemesRoutes[currentPageIndex + 1];
  ...
  return (
    ...
    >
      ...
      >
        <ScrollArea>
          <Flex
            css={{ alignItems: 'center', p: '$4', '@bp2': { display: 'none' } }}
          >
            <Box css={{ ml: '-$1' }}>
              <Button
                ghost
