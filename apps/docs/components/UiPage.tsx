...
import { allUiRoutes, uiRoutes, RouteProps } from '@lib/uiRoutes';
import { NavHeading, NavItem } from './DocNav';

export function UiPage({ children }: { children: React.ReactNode }) {
  ...
  const REPO_NAME = 'coniel/minddrop';
  let editUrl: string = '';

  if (typeof routerSlug === 'string') {
    currentPageSlug = router.pathname.substr(1).replace('[slug]', routerSlug);
    editUrl = `${GITHUB_URL}/${REPO_NAME}/edit/main/packages/docs/data/${currentPageSlug}.mdx`;
  } else {
    currentPageSlug = router.pathname
      .substr(1)
      .replace('[...slug]', routerSlug[0]);
    editUrl = `${GITHUB_URL}/${REPO_NAME}/edit/main/packages/docs/data/${currentPageSlug}/${routerSlug[1]}.mdx`;
  }
  editUrl = editUrl.replace('/docs', '');

  const currentPageIndex = allUiRoutes.findIndex(
    (page) => page.slug === currentPageSlug,
  );
  const previous = allUiRoutes[currentPageIndex - 1];
  const next = allUiRoutes[currentPageIndex + 1];
    ...
    >
      ...
      >
        <ScrollArea>
          ...
          >
            {uiRoutes.map((section: RouteProps) => (
                ...
                {section.pages.map((page) => {
                  ...
                  return (
                    ...
                    >
                      <Text
                        size="2"
                        css={{ color: 'inherit', lineHeight: '1' }}
                      >
                        {page.title}
                      </Text>
                      {isDraft ? (
                        <Badge css={{ ml: '$2' }}>Coming soon</Badge>
                      ) : null}
                      {isDeprecated ? (
