...
- **Extension uninstall**: When an extension which registers a block element type is uninstalled, the elements of that type are converted into `RichText` nodes with the plain text set as their text value.
...
Non-void elements must have a `children` field consisting of `RichText` and `RichTextInlineElement`s nodes.
...
When an element is converted to another type, its `children` field is preserved. For an element to support conversion, it must contain a `children` field (consiting of `RichText` and `RichTextInlineElement` nodes). Other fields may or may not be preserved depending on the data structure of the element type into which the element is converted.
...
<ArgsTable
  data={[
    ...
    {
      name: 'children',
      type: '(RichText | RichTextInlineElement)[] | null',
      required: true,
      description: (
        <span>
