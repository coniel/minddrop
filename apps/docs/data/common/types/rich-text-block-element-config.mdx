---
metaTitle: RichTextElementConfig Interface
metaDescription: Configuration for registering rich text element types.
---

# RichTextElementConfig

<Description>
  Configuration for registering rich text element types.
</Description>
...
<InterfaceTable
  data={[
    ...
    },
    {
      ...
      required: true,
      description:
        'Function called to create an element of this type.',
    },
    {
      name: 'convert',
      type: 'function',
...
Called to create a new rich text element of this type. Should return an object containing the element `type` as well as any data used by the element.
...
- **Extension uninstall**: When an extension which registers a block element type is uninstalled, the elements of that type are converted into `RichTextNode`s with the plain text set as their text value.
...
Non-void elements must have a `children` field consisting of `RichTextNode`s and `RichTextInlineElement`s.
...
When an element is converted to another type, its `children` field is preserved. For an element to support conversion, it must contain a `children` field (consiting of `RichTextNode`s and `RichTextInlineElement`s). Other fields may or may not be preserved depending on the data structure of the element type into which the element is converted.
...
<ArgsTable
  data={[
    ...
    {
      name: 'children',
      type: '(RichTextNode| RichTextInlineElement)[] | null',
      required: true,
      description: (
        <span>
