...
<EventsTable
  data={[
    {
      name: 'persistent-store:create-global',
      data: 'PersistentStoreDocument',
      description:
        'Dispatched when the global store document is created.',
    },
    {
      name: 'persistent-store:create-local',
      data: 'PersistentStoreDocument',
      description:
        'Dispatched when the local store document is created.',
    },
    {
...
An object containing the store document ID an a `data` attribute consisting of key-value pairs namespaced by extension ID.
...
An object describing the changes applied to the store document. Makes use of [`FieldValue`](/docs/extensions/api/utils#fieldvalue) mutations to update the nested namespaced data. The values themselves can also consist of `FieldValue` mutations.
