...
```ts
useSelectableTopic(topicId: string): TopicSelection
```
...
Adds drops to the selected drops list and dispatches a `app:selection:select-drops` event.
...
Removes drops from the selected drops list and dispatches a `app:selection:unselect-drops` event.
...
Adds topics to the selected topics list and dispatches a `app:selection:select-topics` event.
...
Removes topics from the selected topics list and dispatches a `app:selection:unselect-topics` event.
...
Clears the selected drops and topics. Dispatches a `app:selection:clear` event.
