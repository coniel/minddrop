...
Returns an `{ [id]: Topic }` map of root level topics. Returned topics can be filtered by passing in `TopicFilters`.

```ts
useRootTopics(filters?: TopicFilters): TopicMap
```
  ...
  data={[
    {
      ...
      description: (
        <span>
          ...
          <a href="/docs/extensions/api/topics#topicfilters">
            TopicFilters
          </a>{' '}
          section of the Topics interface page for more details.
        </span>
      ),
    },
  ]}
...
Returns an `{ [id]: Drop }` map of selected drops.
...
```ts
App.permanentlyDeleteTopic(core: Core, topicId: string): Topic;
```
...
Adds topics to the root level and dispaches an `app:add-root-topics` event. Returns the deleted topic.
...
Clears the selected drops from the store and dispatches a `app:clear-selected-drops` event.
