...
By default, archived and deleted topics are filtered out. If either `archived` or `deleted` is set to `true`, active topics will be filtered out by default unless `active` is also set to `true`.

A topic is considered active if it is neither archived nor deleted.
