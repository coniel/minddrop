...
Throws a `RichTextElementTypeNotRegisteredError` if the element type is not registered. Throws a `RichTextElementValidationError` if the element contains invalid data or is missing required fields.
