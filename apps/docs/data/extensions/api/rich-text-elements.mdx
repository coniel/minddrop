...
4. If converting to a void element type, `children` is removed. Conversly, if converting from a void element type to a non-void element type, `children` (consisting of a `RichTextFragment` containing the original element's plain text value) is added.
