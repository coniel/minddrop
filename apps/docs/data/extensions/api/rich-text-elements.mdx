...
Returns a `{ [id]: RichTextElement }` map of rich text elements by ID. If a requested element does not exist, it is ommited from the map.

Optionally, the returned elements can be filtered by passing in `RichTextElementFilters`. See the [RichTextElementFilters](#richtextelementfilters) section for filtering options.

```ts
useRichTextElements<T extends RichTextElement>(elementIds: string[], filters?: RichTextElementFilters): RichTextElementMap<T>;
```
...
RichTextElements.initializeData<TData = {}>(
  type: string,
  fromData?: DataInsert | RichTextFragment
): TData
```
...
<ArgsTable
  data={[
    ...
    {
      name: 'fromData',
      type: 'DataInsert | RichTextFragment',
      description: (
        <div>
          ...
          <p>
            ...
            must be a <Code>RichTextFragment</Code>.
          </p>
        </div>
      ),
    },
...
Creates a new rich text element of the given type. Dispatches a `rich-text-elements:create` event and returns the newly created element.
...
Converts a rich text block element to a different type. Dispatches a `rich-text-elements:update` event and returns the updated element.
...
- Throws a `RichTextElementTypeNotRegistered` if the new element type is not registered
- Throws a `RichTextElementNotFoundError` if the element does not exist.
...
>(core: Core, element: RichTextBlockElement, type: string): TElement
```
...
Adds files to a rich text element and dispaches a `rich-text-elements:add-files` event. Returns the updated rich text element.
...
Removes files from a rich text element and dispatches a `rich-text-elements:remove-files`. Returns the updated rich text element.
...
Replaces files in a rich text element (removing current files and adding given files) and dispatches a `rich-text-elements:replace-files` event. Returns the updated rich text element.
...
Converts rich text elements to a plain text string. Void elements are converted using their `toPlainText` method. If they do not have such a method, they are omited.
  ...
  data={[
    {
      ...
      required: true,
      description:
        'The rich text elements to convert into plain text.',
    },
  ]}
/>
...
<EventsTable
  data={[
    ...
    {
      name: 'rich-text-elements:add-parents',
      data: 'AddParentsToRichTextElementEventData',
      description:
        'Dispatched when parents are added to a rich text element.',
    },
    {
      name: 'rich-text-elements:remove-parents',
      data: 'RemoveParentsFromRichTextElementEventData',
      description:
        'Dispatched when parents are removed from a rich text element.',
    },
    {
      name: 'rich-text-elements:nest',
      data: 'NestRichTextElementEventData',
      description:
        'Dispatched when rich text elements are nested into another element.',
    },
    {
      name: 'rich-text-elements:unnest',
      data: 'UnnestRichTextElementEventData',
      description:
        'Dispatched when rich text elements are unnested from another element.',
    },
    {
      name: 'rich-text-elements:add-files',
      data: 'AddFilesToRichTextElementEventData',
      description:
        'Dispatched when files are added to a rich text element.',
    },
    {
      name: 'rich-text-elements:remove-files',
      data: 'RemoveFilesFromRichTextElementEventData',
      description:
        'Dispatched when files are removed from a rich text element.',
    },
    {
      name: 'rich-text-elements:replace-files',
      data: 'ReplaceFilesInRichTextElementEventData',
      description:
