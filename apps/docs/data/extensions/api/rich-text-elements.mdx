...
Retrieves rich text elements by ID. If provided a single ID string, returns the matching rich text element. If provided an array of IDs, returns a `{ [id]: RichTextElement }` of the corresponding rich text elements.

Throws a `RichTextElementNotFoundError` if a element does not exist.

```ts
RichTextElements.get<T extends RichTextElement>(elementId: string | string[]): T | RichTextElementMap<T>
```
...
Retrieves all rich text elements as a `RichTextElementMap`.

```ts
RichTextElements.getAll<T extends RichTextElement>(): RichTextElementMap<T>
```

### create

Creates a new rich text element and dispaches a `rich-text-elements:create` event. Returns the new rich text element.

```ts
RichTextElements.create<
  D extends CreateRichTextElementData,
...
<InterfaceTable
  data={[
    ...
    {
      name: 'parent',
      type: 'RichTextElementParentReference',
      required: true,
      description: (
        <span>
          An object containing the parent type (either 'document' or
          'element') and the parent ID. See the{' '}
          <a href="/docs/extensions/types/rich-text-element#richtextelementparentreference">
            RichTextElementParentReference
          </a>{' '}
    ...
    {
      name: 'children',
      type: '(RichTextNode| InlineElementReference)[]',
      description: (
        <span>
          The rich text content of the element. Can be a combination
          of <Code>RichTextNode</Code>s and{' '}
          <Code>InlineElementReference</Code>s. See the{' '}
          <a href="/docs/extensions/types/rich-text-element#richtext">
            RichTextElement
          </a>{' '}
          page for details.
    ...
    },
    {
      name: 'nestedElements',
      type: 'string[]',
      description: <span>The IDs of nested <Code>RichTextElement</Code>s.</span>
    }
...
RichTextElements.update<
  D extends UpdateRichTextElementData,
  T extends RichTextElement,
>(core: Core, elementId: string, data: D): T
```
...
```ts
RichTextElements.delete(core: Core, elementId: string): RichTextElement
```
...
Restores a deleted rich text element and dispatches a `rich-text-elements:restore` event and an `rich-text-elements:update` event. Returns the restored element.

```ts
RichTextElements.restore(core: Core, elementId: string): RichTextElement
```
...
```ts
RichTextElement.deletePermanently(core: Core, elementId: string): RichTextElement
```
...
<InterfaceTable
  data={[
    ...
    {
      name: 'children',
      type: '(RichTextNode| RichTextElement)[] | Delete',
