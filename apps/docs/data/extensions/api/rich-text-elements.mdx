...
Creates a new rich text element of the given type by calling the element config's `create` method. Dispatches a `rich-text-elements:create` event and returns the newly created element.
...
RichTextElements.createOfType<
  TElement extends RichTextElement,
>(core: Core, type: string, data?: DataInsert): TElement
```
...
<ArgsTable
  ...
  data={[
    ...
    },
    {
      ...
      type: 'string',
      description:
        'A data insert object from which to create the element.',
    },
  ]}
/>
